...
from numpy import ndarray, array, zeros, asarray, asmatrix
...
if platform.system() == 'Windows':
    _sofalib_filename = _glob.glob(str(_pathlib.Path(__file__).parent.parent.parent.joinpath('lib','sofa.dll')))
elif platform.system() == 'Linux':
    _sofalib_filename = _glob.glob(str(_pathlib.Path(__file__).parent.parent.parent.joinpath('lib','sofa.so')))

if len(_sofalib_filename) == 0:
    raise ImportError('Unable to find the shared C lirary "sofa_c".')

_sofa = cdll.LoadLibrary(_sofalib_filename[0])
...
def a2af(ndp, angle):
    ...
    return sign.value.decode(), tuple([v for v in idmsf])
...
def a2tf(ndp, angle):
    ...
    return sign.value.decode(), tuple([v for v in ihmsf])
...
def af2a(s, ideg, iamin, asec):
    ...
    rad = c_double()
    s = _sofa.iauAf2a(c_char(s.encode()), ideg, iamin, asec, byref(rad))
    if s != 0:
...
def d2dtf(scale, ndp, d1, d2):
    ...
    id = c_int()
    ihmsf = (c_int * 4)()
    s = _sofa.iauD2dtf(c_char_p(scale.encode()), ndp, d1, d2, byref(iy), byref(im), byref(id), ihmsf)
    if s < 0:
...
def dtf2d(scale, iy, im, id, ihr, imn, sec):
    ...
    d1 = c_double()
    d2 = c_double()
    s = _sofa.iauDtf2d(c_char_p( scale.encode() ), iy, im, id, ihr, imn, sec, byref(d1), byref(d2))
    if s < 0:
...
def tf2a(s, ihour, imin, sec):
    ...
    if __sofa_version < (2010, 12, 1):
        raise NotImplementedError
    rad = c_double()
    s = _sofa.iauTf2a(c_char(s.encode()), ihour, imin, sec, byref(rad))
    return rad.value
...
def tf2d(s, ihour, imin, sec):
    ...
    if __sofa_version < (2010, 12, 1):
        raise NotImplementedError
    days = c_double()
    s = _sofa.iauTf2d(c_char(s.encode()), ihour, imin, sec, byref(days))
    return days.value
